...
const featuredBlackHoles = [
  ...
  },
  {
    ...
    notes:
      "Primer agujero negro fotografiado directamente por el Event Horizon Telescope.",
    source: "Ficha destacada",
  },
  {
    id: "ton-618",
  ...
  },
  {
    ...
    massSolar: 33,
    notes:
      "Agujero negro estelar masivo detectado por el movimiento de su estrella compañera.",
    source: "Ficha destacada",
  },
];
...
const fallbackCatalog = [
  {
    ...
    distanceKpcText: "3.4±0.3",
    distanceKpc: 3.4,
    distanceLy: 11089,
    orbitalPeriodHours: "10.804±0.001",
    peakFlux: "1.65E-07",
    source: "Fallback local",
  },
  {
    id: "63",
    name: "MAXI J1820+070 / ASASSN-18ey",
    type: "Binario de rayos X / candidato estelar",
    distanceKpcText: "2.96±0.33",
    distanceKpc: 2.96,
    distanceLy: 9654,
    orbitalPeriodHours: "16.4518±0.0002",
    peakFlux: "2.26E-07",
    source: "Fallback local",
  },
  {
    ...
    name: "MAXI J1305-704",
    type: "Binario de rayos X / candidato estelar",
    distanceKpcText: "7.5+1.8-1.4",
    distanceKpc: 7.5,
    distanceLy: 24462,
    orbitalPeriodHours: "9.5±0.1",
    peakFlux: "1.03E-09",
    source: "Fallback local",
  },
  {
    ...
    type: "Binario de rayos X / candidato estelar",
    distanceKpcText: "1.72±0.1",
    distanceKpc: 1.72,
    distanceLy: 5610,
    orbitalPeriodHours: "4.0784088",
    peakFlux: "4.99E-10",
    source: "Fallback local",
  },
  {
    ...
    name: "GRO J1655-40",
    type: "Binario de rayos X / candidato estelar",
    distanceKpcText: "3.2±0.2",
    distanceKpc: 3.2,
    distanceLy: 10437,
    orbitalPeriodHours: "62.926272",
    peakFlux: "1.15E-07",
    source: "Fallback local",
  },
  {
    ...
    name: "GRS 1915+105",
    type: "Binario de rayos X / candidato estelar",
    distanceKpcText: "9.4±0.8",
    distanceKpc: 9.4,
    distanceLy: 30659,
    orbitalPeriodHours: "812±4",
    peakFlux: "9.29E-08",
    source: "Fallback local",
  },
  {
    id: "15",
    name: "GS 2023+338 / V404 Cyg",
    type: "Binario de rayos X / candidato estelar",
    distanceKpcText: "2.39±0.14",
    distanceKpc: 2.39,
    distanceLy: 7795,
    orbitalPeriodHours: "155.30803",
    peakFlux: "2.59E-07",
    source: "Fallback local",
  },
  {
    id: "7",
    name: "3A 0620-003 / V616 Mon",
    type: "Binario de rayos X / candidato estelar",
    distanceKpcText: "1.06±0.1",
    distanceKpc: 1.06,
    distanceLy: 3457,
    orbitalPeriodHours: "7.752340",
    peakFlux: "2.65E-06",
    source: "Fallback local",
  },
  {
...
const distanceInsight = (distanceLy) => {
  if (!distanceLy)
    return "No hay distancia confiable publicada para este objeto, así que conviene compararlo por nombre, coordenadas y fuente antes de sacar conclusiones.";
  if (distanceLy <= 5000)
    return "Está relativamente cerca en escala galáctica. Es más fácil estudiarlo con detalle porque su luz llega con menos debilitamiento y menor confusión con objetos de fondo.";
  if (distanceLy <= 25000)
    return "Está dentro de nuestra galaxia. A esa distancia todavía puede estudiarse bien, pero las mediciones dependen más de modelos y del comportamiento de su estrella compañera.";
  if (distanceLy <= 100000)
    return "Está en escala de Vía Láctea o alrededores. La distancia ya vuelve más inciertas algunas propiedades observadas, sobre todo brillo real y energía emitida.";
...
const massInsight = (massSolar) => {
  if (!massSolar)
    return "La masa no está disponible en este registro. En muchos candidatos se infiere de forma indirecta observando el movimiento de una estrella compañera o la emisión de rayos X.";
  if (massSolar < 100)
    return "Su masa encaja con un agujero negro estelar: probablemente nació del colapso de una estrella masiva. Su tamaño físico puede ser pequeño, pero su gravedad cercana es extrema.";
  if (massSolar < 1000000)
    return "Está en un rango intermedio poco común. Estos objetos son importantes porque podrían conectar agujeros negros estelares con supermasivos.";
  if (massSolar < 1000000000)
    return "Es supermasivo. Normalmente vive en el centro de una galaxia y puede influir en la dinámica de millones de estrellas.";
...
const orbitalInsight = (period) => {
  ...
  if (!text)
    return "Sin periodo orbital publicado. Eso puede significar que todavía no se midió bien el sistema compañero.";
  const value = Number(text.replace(",", ".").match(/\d+(?:\.\d+)?/)?.[0]);
  if (!Number.isFinite(value))
    return "El periodo está publicado con formato astronómico, pero requiere interpretación manual por la fuente original.";
  if (value < 24)
    return "Periodo corto: el objeto compañero está muy cerca del agujero negro, por eso puede transferir material y generar rayos X intensos.";
  if (value < 240)
    return "Periodo medio: el sistema binario es más amplio, pero todavía puede haber intercambio de material hacia el agujero negro.";
...
export function BlackHoles({ data = [] }) {
  ...
  const typeOptions = useMemo(
    () => [
      "Todos",
      ...new Set(catalog.map((item) => item.type).filter(Boolean)),
    ],
    [catalog],
  );
  ...
  const filteredCatalog = useMemo(() => {
    ...
    return catalog
      .filter((item) => {
        const matchesSearch =
          !normalizedSearch ||
          [item.name, item.location, item.ra, item.dec, item.notes, item.source]
            .filter(Boolean)
            .join(" ")
            .toLowerCase()
    ...
    <Container className="my-5 pt-5">
      ...
      <div className="text-center mb-5">
        ...
        <p className="lead text-white-50 mx-auto" style={{ maxWidth: 820 }}>
          Catálogo ampliado combinando destacados curados, BlackCAT y SIMBAD,
          más imágenes actualizadas desde NASA Image and Video Library.
        </p>
      </div>
      ...
      <Tabs defaultActiveKey="catalogo" className="mb-4" fill>
        <Tab eventKey="catalogo" title="Catálogo filtrable">
          ...
          >
            <strong>Lectura rápida:</strong> cuanto más lejos está un agujero
            negro, más antigua es la luz que vemos y más difícil es medir sus
            propiedades. La masa indica la familia del objeto: estelar si tiene
            decenas de masas solares, supermasivo si tiene millones o miles de
            millones. En binarias de rayos X, un periodo orbital corto suele
            indicar una estrella compañera muy cercana que puede alimentar al
            agujero negro.
          </Alert>
          ...
          {catalogLoading ? (
            <div className="text-center py-5">
              ...
              <p className="text-info mt-3">Cargando catálogo...</p>
            </div>
          ) : (
              ...
              <tbody>
                {filteredCatalog.map((item) => (
                  <tr key={`${item.source}-${item.id}-${item.name}`}>
                    ...
                    <td>
                      {formatLy(item.distanceLy)}
                      {item.distanceKpcText ? (
                        <div className="text-white-50 small">
                          {cleanText(item.distanceKpcText)} kpc
                        </div>
                      ) : null}
                    </td>
        ...
        <Modal.Body>
          <Row className="g-3">
            ...
            <Col md={6}>
              ...
              <br />
              {cleanText(selected?.orbitalPeriodHours) || "No disponible"}
            </Col>
            <Col md={6}>
            ...
            <Col md={6}>
              <strong>Flujo pico:</strong>
              <br />
              {cleanText(selected?.peakFlux) || "No disponible"}
            </Col>
            <Col xs={12}>
            ...
            <Col xs={12}>
              ...
              <p className="text-white-50 mt-2">
                {selected?.notes ||
                  "Objeto importado desde catálogo astronómico. Algunas propiedades pueden estar vacías si la fuente no las publica para ese objeto."}
              </p>
            </Col>
          </Row>
        </Modal.Body>
